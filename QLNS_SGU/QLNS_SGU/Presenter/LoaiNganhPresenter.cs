...
namespace QLNS_SGU.Presenter
{
    ...
    public class LoaiNganhPresenter : ILoaiNganhPresenter
    {
        ...
        public void MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                if (hinfo.Column == _view.GVLoaiNganh.Columns[0])
                {
                    _view.GVLoaiNganh.Columns[0].OptionsColumn.AllowEdit = true;
        ...
        {
            _view.GVLoaiNganh.Columns[0].OptionsColumn.AllowEdit = false;
        ...
        public void SaveData()
        {
            ...
            if (!unitOfWorks.LoaiNganhRepository.CheckExistById(idRowFocused))
            {
                string idloainganh = _view.GVLoaiNganh.GetFocusedRowCellDisplayText("idLoaiNganh").ToString();
                int id = -1;
                if (!string.IsNullOrEmpty(idloainganh))
                {
                    id = Convert.ToInt32(idloainganh);
                }
                else
                    XtraMessageBox.Show("Bạn chưa nhập Mã loại ngành.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                string loainganh = _view.GVLoaiNganh.GetFocusedRowCellDisplayText("tenLoaiNganh").ToString();
                if (loainganh != string.Empty)
                {       
                    if (!unitOfWorks.LoaiNganhRepository.CheckExistById(id))
                    {
                        unitOfWorks.LoaiNganhRepository.Create(id, loainganh);
                        LoadDataToGrid();
                        ...
                        XtraMessageBox.Show("Mã loại ngành đã tồn tại. Vui lòng nhập mã khác.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
        ...
        {            
            ...
            gridView.SetRowCellValue(e.RowHandle, gridView.Columns[0], -1);
