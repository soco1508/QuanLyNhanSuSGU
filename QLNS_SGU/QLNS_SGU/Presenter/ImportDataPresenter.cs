            ...
            if (!checkFileNameIsEmpty)
            {
                ...
                if (sheetName.Equals("ChucVuDonVi"))
                {
                    if (unitOfWorks.DonViRepository.CheckHasRows() > 0)
                    {
                        ...
                        List<string> listTenToChuyenMonVaDonVi = null;
                        if (unitOfWorks.ToChuyenMonRepository.CheckHasRows() > 0)
                            listTenToChuyenMonVaDonVi = unitOfWorks.ToChuyenMonRepository.GetListTenToChuyenMonVaDonVi();
                        else
                            listTenToChuyenMonVaDonVi = new List<string>();
                        if (!listTenToChuyenMonVaDonVi.Any(x => x == string.Empty))
                        {
                            unitOfWorks.ToChuyenMonRepository.Insert(new ToChuyenMon
                            {
                                tenToChuyenMon = string.Empty,
                                idDonVi = unitOfWorks.DonViRepository.GetIdDonVi(string.Empty)
                            });
                            listTenToChuyenMonVaDonVi.Add(string.Empty);
                        }
                        int line = 0;
                        int lines = rows.Count();
                        ...
                        foreach (var row in rows)
                        {
                            ...
                            string temp = tochuyenmon + donvi;

                            if (!listTenToChuyenMonVaDonVi.Any(x => x.ToLower() == temp.ToLower()))
                            {
                                unitOfWorks.ToChuyenMonRepository.Insert(new ToChuyenMon
                                {
                                ...
                                listTenToChuyenMonVaDonVi.Add(temp);
                            }
                        }
                        ...
                        XtraMessageBox.Show("Vui lòng import Đơn vị trước.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                    XtraMessageBox.Show("Vui lòng chọn sheet ChucVuDonVi.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
                        ...
                        && unitOfWorks.DonViRepository.CheckHasRows() > 0 && unitOfWorks.ToChuyenMonRepository.CheckHasRows() > 0)
                    {
                        ...
                        foreach (var row in rows)
                        {
                            ...
                            if (idvienchuc > 0)
                            {
                                ...
                                int idtochuyenmon = unitOfWorks.ToChuyenMonRepository.GetIdByIdDonViVaTenToChuyenMon(iddonvi, tochuyenmon);
